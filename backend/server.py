...
# ==================== GÉNÉRATION D'INDISPONIBILITÉS ROUTES MIGRÉES VERS routes/generation_indisponibilites.py ====================
# Routes et fonctions migrées:
# - POST   /{tenant_slug}/disponibilites/generer    - Générer les indisponibilités automatiques
# - generer_indisponibilites_montreal()
# - generer_indisponibilites_quebec()
# - generer_indisponibilites_longueuil()
# ============================================================================
            ...
            for day_offset in range(7):
                ...
                for user in users:
                    ...
                    if user.get("statut") != "Actif":
                        if is_jf_tardif:
                            logging.info(f"    ❌ [JF TARDIF] EXCLU: Statut non actif ({user.get('statut')})")
                        continue
                    ...
                    if type_garde.get("est_garde_externe", False):
                        ...
                        if activer_heures_sup:
                            ...
                            # Pour les gardes INTERNES uniquement : vérifier heures_max_semaine
                            # Pour les gardes EXTERNES : pas de vérification heures (ce ne sont pas des heures travaillées)
                ...
                for iteration_idx in range(places_restantes):
                    ...
                    logging.info(f"✅ [ASSIGNATION] {selected_user['prenom']} {selected_user['nom']} assigné à {type_garde['nom']} ({type_garde_str}, {duree}h) le {date_str}")
...
async def update_niveaux_attribution(
    ...
    if update_fields:
        ...
        logging.info(f"✅ Niveaux d'attribution mis à jour pour {tenant.slug}: {update_fields}")
...
# ==================== POINTS D'EAU ROUTES MIGRÉES VERS routes/points_eau.py ====================
# Routes migrées:
...
class PointEauBase(BaseModel):
    ...
    adresse: Optional[str] = None
    ville: Optional[str] = None
    secteur_id: Optional[str] = None  # ID du secteur géographique
    notes: Optional[str] = None
...
class BorneFontaine(PointEauBase):
    ...
    date_derniere_inspection: Optional[str] = None
    frequence_inspection_mois: int = 12  # Inspection annuelle par défaut
    ...
    for point in points:
        ...
        if not date_derniere:
            ...
            if point.get('etat') in ['defectueux', 'inaccessible']:
                ...
                point['statut_couleur'] = 'orange'  # Inspection à venir (<30j)
            else:
                point['statut_couleur'] = 'vert'  # Conforme
        
        # Pour bornes sèches, vérifier aussi la date de test programmée
        if point.get('type') == 'borne_seche' and point.get('prochaine_date_test'):
            try:
                ...
                if jours_avant_test < 0:
                    ...
                    point['statut_couleur'] = 'orange'  # Test à venir
            except:
                pass
...
async def create_point_eau(
    ...
    # Vérifier permissions (admin ou preventionniste)
    ...
    # Définir fréquence par défaut selon le type
...
async def create_inspection_point_eau(
    ...
    """Créer une nouvelle inspection de point d'eau"""
    tenant = await get_tenant_from_slug(tenant_slug)
    ...
    # Mettre à jour la date de dernière inspection du point d'eau
    ...
    return {"message": "Inspection créée avec succès", "id": inspection_dict['id']}
...
async def get_inspections_points_eau(
    ...
    """Récupérer les inspections de points d'eau"""
    tenant = await get_tenant_from_slug(tenant_slug)
...
async def get_statistiques_approvisionnement(
    ...
    """Récupérer les statistiques du module approvisionnement en eau"""
    tenant = await get_tenant_from_slug(tenant_slug)
    ...
    # Inspections récentes (30 derniers jours)
    from datetime import datetime, timedelta
