...
@router.post("/{tenant_slug}/auth/diagnose-login")
async def diagnose_login(tenant_slug: str, login: LoginRequest):
    """
    Endpoint de diagnostic pour comprendre les échecs de login.
    Retourne des informations détaillées sans révéler le hash complet.
    """
    import logging
    logger = logging.getLogger(__name__)
    ...
    diagnosis = {
        "tenant_slug": tenant_slug,
        "tenant_id": tenant.id,
        "email_provided": login.email,
        "password_length": len(login.mot_de_passe) if login.mot_de_passe else 0,
        "user_found": False,
        "hash_exists": False,
        "hash_type": None,
        "hash_valid_format": False,
    ...
    if user:
        diagnosis["user_found"] = True
        diagnosis["account_active"] = user.get("actif", True)
        
        stored_hash = user.get("mot_de_passe_hash", "")
        
        if stored_hash:
            diagnosis["hash_exists"] = True
            diagnosis["hash_length"] = len(stored_hash)
            
            if stored_hash.startswith("$2a$") or stored_hash.startswith("$2b$"):
                diagnosis["hash_type"] = "bcrypt"
                diagnosis["hash_valid_format"] = True
            elif stored_hash.startswith("$2"):
                diagnosis["hash_type"] = "bcrypt_variant"
                diagnosis["hash_valid_format"] = True
            else:
                diagnosis["hash_type"] = "unknown"
                diagnosis["hash_prefix"] = stored_hash[:10] if len(stored_hash) > 10 else stored_hash
            
            # Tester la vérification
            try:
                diagnosis["password_verified"] = verify_password(login.mot_de_passe, stored_hash)
            except Exception as e:
                diagnosis["verification_error"] = str(e)
        else:
            diagnosis["hash_exists"] = False
            diagnosis["error"] = "Aucun hash de mot de passe stocké"
    else:
        ...
        if super_admin:
            diagnosis["is_super_admin"] = True
            stored_hash = super_admin.get("mot_de_passe_hash", "")
            
            if stored_hash:
                diagnosis["hash_exists"] = True
                diagnosis["hash_length"] = len(stored_hash)
                diagnosis["hash_type"] = "bcrypt" if stored_hash.startswith("$2") else "unknown"
    ...
    logger.info(f"[DIAGNOSE] Résultat: {diagnosis}")
