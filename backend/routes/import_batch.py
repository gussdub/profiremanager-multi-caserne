...
# ======================== SYNC DONNÉES PFM → USERS (ONE-TIME) ========================

def _pfm_noms_to_app(pfm_raw) -> list:
    """Convertit les nominations PFM brutes en [{titre, date_obtention}]."""
    if not pfm_raw:
        return []
    items = []
    if isinstance(pfm_raw, dict):
        raw = pfm_raw.get("nomination") or pfm_raw.get("item") or []
        items = [raw] if isinstance(raw, dict) else (raw if isinstance(raw, list) else [])
    elif isinstance(pfm_raw, list):
        items = pfm_raw
    result = []
    for item in items:
        if isinstance(item, dict):
            date_raw = item.get("date_nomin") or item.get("date_debut") or item.get("date_obtention") or ""
            titre = item.get("grade") or item.get("titre") or item.get("type_nomination") or ""
            result.append({"titre": titre, "date_obtention": date_raw[:10] if date_raw else ""})
    return result


@router.post("/{tenant_slug}/import/sync-pfm-user-data")
async def sync_pfm_user_data(
    tenant_slug: str,
    current_user: User = Depends(get_current_user),
):
    """One-shot : synchronise NAS, code_permanent, adresse, nominations, grade depuis
    imported_personnel.pfm_record vers users pour tous les comptes PFM liés.
    Nettoie aussi les faux emails @import.local. À supprimer après utilisation."""
    tenant = await get_tenant_from_slug(tenant_slug)

    users_pfm = await db.users.find(
        {"tenant_id": tenant.id, "imported_from_pfm": True}, {"_id": 0}
    ).to_list(None)
    ...
    for u in users_pfm:
        try:
            pid = u.get("imported_personnel_id")
            if not pid:
                stats["skipped"] += 1
                continue

            emp = await db.imported_personnel.find_one({"id": pid, "tenant_id": tenant.id}, {"_id": 0})
            if not emp:
                stats["skipped"] += 1
                continue

            pfm = emp.get("pfm_record") or {}

            # ── Adresse complète depuis pfm_record.adresse ─────────────────
            adresse_pfm = pfm.get("adresse") or {}
            if isinstance(adresse_pfm, dict) and "adresse" in adresse_pfm:
                adresse_pfm = adresse_pfm["adresse"]
            adresse_update = ""
            if isinstance(adresse_pfm, dict):
                no_civ = str(adresse_pfm.get("no_civ") or "").strip()
                type_rue = str(adresse_pfm.get("type_rue") or "").strip()
                rue = str(adresse_pfm.get("rue") or "").strip()
                ville = str(adresse_pfm.get("ville") or "").strip()
                code_post = str(adresse_pfm.get("code_post") or adresse_pfm.get("code_postal") or "").strip()
                province = str(adresse_pfm.get("province") or "").strip()
                rue_full = " ".join(filter(None, [no_civ, type_rue, rue]))
                adresse_update = " ".join(filter(None, [rue_full, ville, province, code_post]))
            ...
            stats["errors"] += 1
            details.append({"nom": f"{u.get('prenom','')} {u.get('nom','')}", "error": str(e)})

    return {"success": True, "tenant": tenant_slug, "stats": stats, "details": details}
