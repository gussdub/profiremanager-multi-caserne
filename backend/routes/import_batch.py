...
async def backfill_employes_accounts(
    ...
    """Migration one-shot : crée les comptes users manquants pour imported_personnel existants."""
    tenant = await get_tenant_from_slug(tenant_slug)
    ...
    for emp in records:
        try:
            ...
            pfm_fields = {
                "imported_from_pfm": True, "imported_personnel_id": doc_id,
                "nas": emp.get("nas", ""), "numero_passeport": emp.get("numero_passeport", ""),
                "pfm_matricule": matricule, "pfm_caserne": emp.get("caserne", ""),
                "pfm_nominations": emp.get("nominations"),
                "pfm_contacts_urgence": emp.get("contacts_urgence"),
    ...
    return {"success": True, "tenant": tenant_slug, "stats": stats,
            "total_processed": len(records), "temp_password": PFM_DEFAULT_PASSWORD, "details": details}
