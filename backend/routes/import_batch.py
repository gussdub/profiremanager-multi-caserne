...
# ======================== BACKFILL COMPTES EMPLOYES (ONE-TIME) ========================

@router.post("/{tenant_slug}/import/backfill-employes-accounts")
async def backfill_employes_accounts(
    tenant_slug: str,
    current_user: User = Depends(get_current_user),
):
    """
    Migration one-shot : crée ou lie les comptes utilisateurs pour tous les
    imported_personnel existants qui n'ont pas encore de compte app.
    - Actif PFM  → crée compte (statut Actif, role employe, mdp Pompier@2024)
    - Inactif PFM → pas de compte créé
    - Email/matricule existant → liaison seulement (statut préservé)
    À supprimer après utilisation.
    """
    tenant = await get_tenant_from_slug(tenant_slug)

    cursor = db.imported_personnel.find({"tenant_id": tenant.id}, {"_id": 0})
    records = await cursor.to_list(None)

    PFM_DEFAULT_PASSWORD = "Pompier@2024"
    stats = {"created": 0, "linked": 0, "skipped_inactive": 0, "already_linked": 0, "errors": 0}
    details = []
    ...
    for emp in records:
        try:
            ...
            # Déjà lié ?
            if emp.get("imported_personnel_id") or await db.users.find_one(
                {"tenant_id": tenant.id, "imported_personnel_id": doc_id}, {"_id": 0, "id": 1}
            ):
            ...
            if existing_user:
                await db.users.update_one({"id": existing_user["id"]}, {"$set": pfm_user_fields})
                stats["linked"] += 1
                details.append({"action": "linked", "nom": f"{prenom} {nom}", "user_id": existing_user["id"]})
            elif pfm_actif:
                ...
                new_user = {
                    ...
                    "prenom": prenom,
                    "role": "employe",
                    "grade": emp.get("pfm_record", {}).get("grade") or emp.get("pfm_record", {}).get("titre") or "",
                ...
                stats["created"] += 1
                details.append({"action": "created", "nom": f"{prenom} {nom}", "email": new_user["email"], "user_id": new_user_id})
            else:
            ...
            stats["errors"] += 1
            details.append({"action": "error", "nom": emp.get("nom", "?"), "error": str(e)})
    ...
    return {
        ...
        "temp_password": PFM_DEFAULT_PASSWORD,
        "details": details,
    }
