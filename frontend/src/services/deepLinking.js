...
const handleDeepLink = (url, navigate, currentTenantSlug) => {
  try {
    ...
    if (currentTenantSlug && tenantSlug !== currentTenantSlug) {
      ...
      // TODO: Gérer le changement de tenant si nécessaire
    }
...
const handleModuleSpecificActions = (module, params) => {
  ...
  switch (module) {
    case 'remplacements':
      if (itemId || highlight) {
        // Déclencher un événement pour que le composant Remplacements puisse
        // ouvrir automatiquement le modal de la demande
