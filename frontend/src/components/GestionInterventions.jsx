...
const InterventionDetailModal = ({ intervention, tenantSlug, user, onClose, onUpdate, toast, readOnly = false }) => {
  ...
  const [showRemisePropriete, setShowRemisePropriete] = useState(false);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="pt-4">
          ...
          {/* Section Propriétaire (si lié à Prévention) */}
          ...
          {/* Niveau de risque (si lié à Prévention) */}
          {formData.risk_level && (
            <div className="mt-4 pt-4 border-t border-gray-200">
              <h4 className="font-medium text-gray-700 mb-2">⚠️ Niveau de risque</h4>
              <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${
                formData.risk_level === 'Élevé' || formData.risk_level === 'Très élevé'
                  ? 'bg-red-100 text-red-700'
                  : formData.risk_level === 'Moyen'
                    ? 'bg-yellow-100 text-yellow-700'
                    : 'bg-green-100 text-green-700'
              }`}>
                {formData.risk_level}
