...
const ParametresRemplacements = ({ tenantSlug }) => {
  const { toast } = useToast();
  const [remplacementSettings, setRemplacementSettings] = useState({
    delai_minimum_heures: 24,
    notification_auto: true,
    priorite_anciennete: true,
    max_remplacements_mois: 5
  });
  ...
  const loadSettings = async () => {
    try {
      const data = await apiGet(tenantSlug, '/parametres/remplacements');
      if (data) setRemplacementSettings(data);
    } catch (error) {
      console.error('Erreur chargement paramètres remplacements:', error);
    }
  };
  ...
  const saveSettings = async () => {
    setSaving(true);
    try {
      await apiPost(tenantSlug, '/parametres/remplacements', remplacementSettings);
