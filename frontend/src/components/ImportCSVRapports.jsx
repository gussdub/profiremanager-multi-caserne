...
const ImportCSVRapports = ({ tenantSlug, onImportComplete }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch(`/api/${tenantSlug}/rapports/budgets/import`, {
        method: 'POST',
        headers: {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>📥 Import CSV - Rapports & Budgets</CardTitle>
        ...
        {step === 1 && (
          <div className="import-step">
            <div className="step-header">
              ...
              <p>Choisissez votre fichier CSV contenant les rapports</p>
            </div>
          ...
          <div className="import-step">
            ...
            <div className="mapping-container">
              <div className="mapping-header" style={{
                ...
                <div>📊 <strong>{csvFile?.name}</strong> - {csvData.length} ligne(s), {csvHeaders.length} colonne(s)</div>
              </div>
              <div style={{
                padding: '1rem', backgroundColor: '#eff6ff', border: '1px solid #bfdbfe',
                borderRadius: '0.5rem', marginBottom: '1.5rem', fontSize: '0.875rem'
              }}>
                <p style={{ margin: '0 0 0.5rem 0' }}>💡 <strong>Astuce:</strong> Utilisez la colonne "Valeur par défaut" pour appliquer une même valeur à toutes les lignes.</p>
                <p style={{ margin: 0, fontSize: '0.8rem', color: '#64748b' }}>⚠️ La valeur par défaut écrase toujours les données du CSV si les deux sont renseignés.</p>
              </div>
              <div style={{ overflowX: 'auto' }}>
                ...
                {availableFields.map(field => (
                  <div key={field.key} style={{
                    ...
                    <div><span style={{ fontWeight: '500', color: '#475569' }}>
                      {field.label}
                    </span></div>
                    <div style={{ textAlign: 'center' }}>➡️</div>
                    <div>
                    ...
                    <div style={{ fontSize: '0.875rem', color: '#64748b' }}>
                      {defaultValues[field.key] ? (
                        <span style={{ fontWeight: '600', color: '#059669' }}>{defaultValues[field.key]} <small>(toutes)</small></span>
                  ...
                  <thead>
                    <tr style={{ backgroundColor: '#f8fafc' }}>
                      ...
                      {availableFields.map(f => (
                        <th key={f.key} style={{ padding: '0.75rem', textAlign: 'left', borderBottom: '2px solid #e2e8f0', whiteSpace: 'nowrap' }}>
                  ...
                  <tbody>
                    {previewData.map((row, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #e2e8f0' }}>
                        <td style={{ padding: '0.75rem', color: '#64748b', position: 'sticky', left: 0, backgroundColor: 'white', zIndex: 1 }}>{i + 1}</td>
                        {availableFields.map(f => (
                          <td key={f.key} style={{ padding: '0.75rem', whiteSpace: 'nowrap' }}>
                            {row[f.key] ? (
                              <span style={{ color: defaultValues[f.key] ? '#059669' : '#0f172a' }}>{row[f.key]}</span>
              ...
              <div>
                {importResults.success ? (
                  <div style={{ padding: '2rem', textAlign: 'center', backgroundColor: '#f0fdf4', borderRadius: '0.5rem' }}>
                    ...
                    <h3>Import réussi !</h3>
                    <p>{importResults.imported_count} rapport(s) importé(s)</p>
