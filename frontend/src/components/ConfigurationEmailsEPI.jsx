...
const ConfigurationEmailsEPI = ({ tenantSlug }) => {
  ...
  const loadConfiguration = async () => {
    try {
      setLoading(true);
      const config = await apiGet(tenantSlug, '/epi/parametres');
      setSelectedUsers(config.emails_notifications_epi || []);
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      await apiPut(tenantSlug, '/epi/parametres', {
        ...config,
        emails_notifications_epi: selectedUsers
      });
...
export default ConfigurationEmailsEPI;
