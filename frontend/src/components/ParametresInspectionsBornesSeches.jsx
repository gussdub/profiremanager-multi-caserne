...
const TYPES_CHAMPS = [
  ...
  { value: 'sketch', label: '🎨 Croquis/Dessin', hasOptions: false },
];
    ...
    >
      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
        ...
        <input
          ...
          placeholder="Nom de l'item (ex: Joint présent)"
          style={{
            flex: 1,
            padding: '0.5rem',
...
const SortableSection = ({ 
  ...
  return (
    ...
    >
      ...
      {/* Type de champ */}
      <div style={{ marginBottom: '1rem', padding: '0.75rem', backgroundColor: '#fecaca', borderRadius: '0.375rem', border: '1px solid #fca5a5' }}>
        ...
        {typeChamp.hasUnit && (
          <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <label style={{ fontSize: '0.75rem', color: '#991b1b', fontWeight: '600' }}>Unité:</label>
            <input
              type="text"
              value={section.unite || ''}
              onChange={(e) => updateSection(sectionIndex, 'unite', e.target.value)}
              placeholder="Ex: °C, PSI, secondes, L/min"
              style={{
                flex: 1,
                padding: '0.375rem',
                border: '1px solid #d1d5db',
            ...
            />
            <label style={{ fontSize: '0.75rem', color: '#991b1b', fontWeight: '600' }}>Seuil alerte:</label>
            <input
              type="number"
              value={section.seuil_alerte || ''}
              onChange={(e) => updateSection(sectionIndex, 'seuil_alerte', e.target.value ? parseFloat(e.target.value) : null)}
        ...
        {typeChamp.hasOptions && (
          <div style={{ marginTop: '0.75rem' }}>
            <label style={{ fontSize: '0.75rem', color: '#991b1b', fontWeight: '600', display: 'block', marginBottom: '0.5rem' }}>
              Options de réponse :
            </label>
            ...
            {(section.options || []).map((opt, optIndex) => (
              <div key={optIndex} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }}>
                <input
                  ...
                  placeholder="Ex: Conforme, Non conforme, N/A..."
                  style={{
                    flex: 1,
                    padding: '0.375rem',
                    border: '1px solid #d1d5db',
                ...
                />
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', whiteSpace: 'nowrap', color: '#dc2626' }}>
                  <input
                    type="checkbox"
...
// Composant principal
const ParametresInspectionsBornesSeches = ({ tenantSlug }) => {
  ...
  const sauvegarderModele = async () => {
    ...
    try {
      ...
      if (modeleEnCours.id) {
        await apiPut(tenantSlug, `/bornes-seches/modeles-inspection/${modeleEnCours.id}`, payload);
      } else {
        await apiPost(tenantSlug, '/bornes-seches/modeles-inspection', payload);
      }
  ...
  const activerModele = async (modeleId) => {
    try {
      await apiPost(tenantSlug, `/bornes-seches/modeles-inspection/${modeleId}/activer`, {});
      fetchModeles();
    } catch (error) {
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#f9fafb', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
          <div style={{ marginBottom: '1rem' }}>
            ...
            <input
              ...
              placeholder="Ex: Inspection standard, Inspection complète..."
              style={{
                width: '100%',
                padding: '0.75rem',
                border: '1px solid #d1d5db',
    ...
    <div style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '600', color: '#111827' }}>
            🔧 Configuration des Inspections de Bornes Sèches
          </h2>
      ...
      {modeles.length === 0 ? (
        <div style={{ padding: '3rem', textAlign: 'center', color: '#6b7280', backgroundColor: '#f9fafb', borderRadius: '0.75rem', border: '2px dashed #d1d5db' }}>
          ...
          <h3 style={{ margin: '0 0 0.5rem', color: '#374151' }}>Aucun modèle d'inspection</h3>
          <p>Créez votre premier modèle pour personnaliser les inspections de bornes sèches.</p>
          <button
            onClick={nouveauModele}
            style={{
              marginTop: '1rem',
          ...
          {modeles.map(modele => (
            <div
              ...
              style={{
                ...
                border: modele.est_actif ? '2px solid #10b981' : '1px solid #e5e7eb',
...
export default ParametresInspectionsBornesSeches;
