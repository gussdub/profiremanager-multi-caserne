...
const SectionRCCI = ({ intervention, tenantSlug, user, getToken, toast, canEdit }) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.rcci) {
            ...
            // Charger toutes les données incluant les nouvelles
            setFormData({
              // 1. Mécanique
              ...
              // 7. Météo (si pas dans RCCI, prendre de l'intervention)
            ...
            // Pas de RCCI existante, initialiser avec météo de l'intervention
            setFormData(prev => ({
              ...prev,
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Convertir les heures de sécurisation en datetime complet
      ...
      if (response.ok) {
        ...
        if (data.requires_transfer_alert) {
          toast({
            ...
            description: "Cette intervention devrait être transférée à la police",
            variant: "warning"
          });
        }
      } else {
  ...
  const addEvidence = () => {
    const newEvidence = {
      ...
      seized_by: user.nom + ' ' + user.prenom,
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          {/* NOUVEAU : Facteurs de propagation */}
          <div>
      ...
      {canEdit && (
        <div style={{ position: 'sticky', top: '1rem', zIndex: 10, textAlign: 'right' }}>
          <Button
            onClick={handleSave}
            disabled={saving}
            style={{
