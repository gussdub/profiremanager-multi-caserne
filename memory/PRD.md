...
- **Badge visuel "Importé"** (amber) pour les importées, aucun badge pour les normales
- **Section "Interventions"** dans la fiche bâtiment : affiche TOUTES les interventions (normales + importées) liées par `batiment_id` ou par correspondance d'adresse
- **Mapping rétroactif automatique** : quand un bâtiment est créé, les interventions orphelines correspondantes sont automatiquement reliées
...
| `/{tenant}/batiments/{id}/interventions-historique` | GET | Interventions liées à un bâtiment (toutes sources) |
| `/{tenant}/import-history/dossier-adresses` | GET | Dossiers d'adresse importés |

### Mapping rétroactif
- Fonction `auto_link_interventions_to_batiment()` dans `import_interventions.py`
- Appelée automatiquement lors de `POST /{tenant}/batiments` (création manuelle)
- Aussi dans `batiments_import.py` via `link_interventions_to_batiment()` (import CSV/XML)
- Couvre les champs `address_full`, `adresse`, `adresse_incident`
...
- **Import historique interventions** : CSV/ZIP, mapping intelligent, chunks, badge visuel, mapping rétroactif (Avr 2026)
