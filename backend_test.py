...
class RapportsExportTester:
    ...
    def test_rapport_competences_specific_user(self):
        """Test GET /api/{tenant_slug}/formations/rapports/competences with user_id (specific user report)"""
        try:
            if not self.admin_token:
                self.log_test("Rapport Competences Specific User", False, "No admin token available")
                return False
            ...
            if response.status_code != 200:
                self.log_test("Rapport Competences Specific User", False, 
                            f"Failed to get users: {response.status_code}")
                return False
            
            users = response.json()
            if not users:
                self.log_test("Rapport Competences Specific User", False, "No users found in Shefford")
                return False
            ...
            if response.status_code == 200:
                ...
                results["missing_fields"] = missing_fields
                results["user_id_matches"] = data.get("user_id") == test_user_id
                results["annee_matches"] = data.get("annee") == 2025
                
                if len(missing_fields) == 0 and results["user_id_matches"] and results["annee_matches"]:
                    success = True
                    message = f"✅ Specific user competences report working - Found {results['response_structure']['competences_count']} competences for user {test_user.get('email')}"
                else:
                    success = False
                    issues = []
                    if missing_fields:
                        issues.append(f"Missing fields: {missing_fields}")
                    if not results["user_id_matches"]:
                        issues.append(f"User ID mismatch")
                    if not results["annee_matches"]:
                        issues.append(f"Year mismatch")
                    message = f"❌ Specific user competences report invalid - {'; '.join(issues)}"
            else:
                success = False
                message = f"❌ Rapport competences specific user failed with status {response.status_code}"
            
            self.log_test("Rapport Competences Specific User", success, message, results)
            return success
